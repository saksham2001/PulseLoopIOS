...
func toCPointer(_ s: String) -> UnsafePointer<Int8>! {
  // `(s as NSString).utf8String` returns a pointer into an autoreleased buffer
  // that is freed when the pool drains. These pointers get stored inside C config
  // structs and read later by onnxruntime (during session create / generate),
  // which is a use-after-free that corrupts the native heap under repeated model
  // loads. Hand out a stable strdup'd copy instead. The copies outlive the C call
  // (a few small per-load allocations); the safety is worth the negligible leak.
